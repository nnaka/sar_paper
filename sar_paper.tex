...
\subsection*{Abstract} \begin{small} Synthetic Aperture Radar (SAR) imagery has
  many applications such as reconnaissance and disaster relief.  Due to the
  importance of speedy information gathering in these applications, the need
  for efficient processing of SAR data grows. A ubiquitous step in SAR image
  formation is autofocus. While previous research has explored effective and
  efficient methods for autofocus, this paper builds upon these works by
  providing efficient implementations of one such algorithm--minimum entropy
  gradient descent--using parallel architectures.  Both a parallel CPU as well
  as GPU implementation are compared to a standard MATLAB implementation
  yielding a significant improvement in performance.
\end{small}

\input{sections/introduction} \input{sections/relatedwork}
\input{sections/implementation} \input{sections/results}
\input{sections/futurework} \input{sections/conclusions}

\begin{small} \bibliographystyle{abbrv} \bibliography{sar_paper} \end{small}
